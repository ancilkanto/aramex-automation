...
if (class_exists('WC_Email')) {
    ...
    public function __construct()
    {
        ...
        // Ensure this email type is considered enabled by WooCommerce filters
    ...
    public function trigger($order_id, $order = false, $tracking_number = '')
    {
        ...
        if (is_a($order, 'WC_Order')) {
            $this->object = $order;
            // Explicitly set recipient to customer's billing email
            ...
            // Fallback: if tracking number wasn't provided to trigger(), try order meta
        ...
        // Abort if disabled or no recipient
        if (!$this->is_enabled() || !$this->get_recipient()) {
            $this->restore_locale();
            return;
        }

        // Force HTML content type during send
